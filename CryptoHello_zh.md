...
**【定义3】** 伪随机函数发生器$seed(uint48 s)$为设置随机数发生器种子，$rand()$为随机数发生器结果，返回48位无符号整数。该函数满足$X_{n+1} = (aX_n + c)\ mod\ m$,  $n \ge 0$，其中$m = 2^{48}$，$a =25214903917$, $c = 11$（glibc的$rand48()$线性同余伪随机数发生器)  
