...
**【定义1】** 单向函数族hi，$0 \le i \le 15$，共计16种不同的单向函数。每个单向函数的输入固定为256位（h0的输入可以是任意长度），输出固定为256位。   
**【定义2】** 工作存储器M，总容量为|M|字节，|M|能被32整除，目前设置在为1M。  
**【定义3】** 伪随机函数发生器seed(uint48 s)为设置随机数发生器种子，rand()为随机数发生器结果，返回48位无符号整数。该函数满足Xn+1 = (aXn + c) mod m,  n ≥0，其中$m = 2^{48}$，a =25214903917, c = 11（glibc的rand48()线性同余伪随机数发生器)  
