...
**【定义3】** 伪随机函数发生器seed(uint48 s)为设置随机数发生器种子，rand()为随机数发生器结果，返回48位无符号整数。该函数满足Xn+1 = (aXn + c) mod m,  n ≥0，其中m = <a href="https://www.codecogs.com/eqnedit.php?latex=2^{48}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?2^{48}" title="2^{48}" /></a>，a =25214903917, c = 11（glibc的rand48()线性同余伪随机数发生器)  
