...
图2-2中，三种显卡的性能始终保持在一个水平上，不受输入数据规模大小的影响。其中1080的核数虽然少于Titan X，但是其显卡频率比Titan X要高一些，两者优势互相抵消，因此两者性能几乎保持一致。Vega的核数虽然多于其它显卡，但是Vega是在windows环境下进行测试的，其相比Linux环境下的性能还是要略低一些。    
与图2-1相比，三种GPU显卡的性能与PC机的性能比较接近。  
